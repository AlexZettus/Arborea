...
local function OnTalk()
	...
	Quest.SetAcceptOptions(5610085);
	Quest.SetCompleteOptions(5610085);
...
local function OnAcceptQuestTalk(id,step)--Если отменить задание, можно принять его заново вручную.
	if( id == 30886)then
  	--  Quest.CloseNPCTalk();
		Quest.AcceptQuestTalk(30886,"<font=Dragon Font,0,19,20><color=0xfffcf157> Посланник Благодарности:<color=0xffffffff>\\n Сейчас время получить опыт в четвертый раз, поторопись. После 21:50 можно будет получить опыт еще раз!");
	end
end
...
RganenshizheD = {}
RganenshizheD["OnTalk"] = OnTalk
RganenshizheD["OnAcceptQuestTalk"] = OnAcceptQuestTalk
RganenshizheD["OnCompleteQuestTalk"] = OnCompleteQuestTalk
