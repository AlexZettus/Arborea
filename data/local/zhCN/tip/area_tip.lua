...
function GetObjString(obj,proto)
	...
	if nlevel ~= nil and  nlevel > 0 then
		...
		if playerlv < nlevel then
			ret = ret .. "\\n \\n" .. "Требуемый уровень: <color=0xffff402d> " .. nlevel .. "<color=0xffffffff>"
		else
			ret = ret .. "\\n \\n" .. "Требуемый уровень: <color=0xff05ff00> " .. nlevel .. "<color=0xffffffff>"
		end
	end
	
	--Требования божественности
	...
	if nnlevel ~= nil and nnlevel > 0 then
		...
		if playerlv < nnlevel then
			ret = ret .. "\\n" .. "Требуемый уровень: <color=0xffff402d> " .. nnlevel .. "<color=0xffffffff>"
		else
			ret = ret .. "\\n" .. "Требуемый уровень: <color=0xff05ff00> " .. nnlevel .. "<color=0xffffffff>"
		end
	end
	...
	if time > 0 then
		ret = ret .."\\n" .. "Действие: <color=0xff05ff00> "
			...
			if time%60 > 1 then
			 ret = ret .. time%60 .. "с.<color=0xffffffff>"
			end
	end
	...
	--Пассивный, уролвень навыка повышается
	if use ~= 2 then
		if nstate ~= 2 then
			--Пустые строки
			ret = ret .. "\\n    "
			...
			if new > 0 then
					ret = ret .. "\\nРадиус действия<color=0xff00a2ff> " .. GetInteger(new/100) .. " <color=0xffffffff>метров"
			end
